...
function ManageAgent() {
  ...
  //const [alertV, setAlertV] = useState("success");
  ...
  const enableAgent = async e => {
    ...
    await axios
      .post(
        "http://localhost:8081/enable-disable-agent",
        { id: agentId },
        {
  ...
  const fetchLocation = async e => {
    // e.preventDefault();
    await axios
      .get("http://localhost:8081/init/location")
      .then(response => {
        setPlans(response.data);
      })
      .catch(function(err) {
      ...
      <section className="filter result-page">
        <div className="container">
          ...
          <Modal show={show} onHide={handleClose}>
            ...
            <Modal.Body>
              Are you sure you want to continue to disable/enable the agent with
              id {agentId}
            </Modal.Body>
            <Modal.Footer>
              ...
              <Button variant="primary" onClick={enableAgent}>
                Continue
              </Button>
            </Modal.Footer>
          </Modal>
